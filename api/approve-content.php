...
/**
 * Approve Content API
 * Approves pending content and moves file from staging to approved
 * Tsharok LMS
 */
...
try {
    ...
    try {
        foreach ($contentIds as $contentId) {
            ...
            if ($content['is_approved'] == 1) {
                $failCount++;
                $errors[] = "Content ID {$contentId} is already approved";
                continue;
            }
            ...
            // Move file from staging to approved (if file exists)
            if ($filename && $filename !== '') {
                $moved = moveToApproved($filename);
                
                if ($moved) {
                    // Update file URL to point to approved directory
                    $newFileUrl = '/uploads/content/' . $filename;
                } else {
                    // File might not exist in staging, but continue with approval
                    $newFileUrl = $content['file_url'];
                    error_log("Warning: Could not move file for content ID {$contentId}, but continuing with approval");
                }
            } else {
                $newFileUrl = $content['file_url'];
            }
            ...
            // Log activity
            logActivity($adminId, 'content_approved', "Admin approved content ID: {$contentId} - {$content['title']}", $db);
        ...
        $message = "Successfully approved {$successCount} content item(s)";
        if ($failCount > 0) {
    ...
    error_log("Approve Content Exception: " . $e->getMessage());
    sendJsonResponse(false, 'An error occurred while approving content: ' . $e->getMessage());
}
?>
